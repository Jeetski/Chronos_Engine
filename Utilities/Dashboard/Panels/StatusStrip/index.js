...
function statusTypes(){
  ...
  const inferred = Object.keys(bundle.current || {}).map(key => key);
  const seen = new Set();
  const list = [];
...
function mountStatusStrip(root, context){
  ...
  function renderGrid(){
    ...
    gridEl.innerHTML = typeDefs.map((def, idx) => {
      const value = state.status[def.key] || '—';
      const accent = STATUS_COLORS[def.key] || colorForIndex(idx, typeDefs.length);
      return `
