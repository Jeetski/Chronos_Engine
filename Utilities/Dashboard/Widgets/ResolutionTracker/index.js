export async function mount(el, context) {
  ...
  async function render() {
    ...
    statsEl.innerHTML = `
      <div style="display:flex; gap:16px; font-size:13px; color:#8a919f;">
        <div style="display:flex; align-items:center; gap:6px;">
          <span>${yearItems.length}</span> <span>total</span>
        </div>
        <div style="display:flex; align-items:center; gap:6px;">
          <strong style="color:#f4d03f; font-weight:600;">${totalProgress}%</strong> <span>overall</span>
        </div>
      </div>
    `;
  ...
  function renderEmpty(container) {
    container.innerHTML = `
      <div style="text-align:center; padding:40px 20px; color:#8a919f;">
        <h3 style="margin:0 0 8px 0; font-size:18px; color:#a8b0c0;">No Resolutions Yet</h3>
        <p style="margin:0; font-size:14px;">Create your first resolutions using the New Year's Resolutions Wizard.</p>
        <p style="margin-top:12px; color:#6a7180; font-size:14px;">Open <strong>Wizards → New Year's Resolutions Wizard</strong> to get started.</p>
      </div>
    `;
  }
  ...
  function injectStyles() {
    ...
    style.textContent = `
      .resolution-card {
        background: linear-gradient(135deg, rgba(218,165,32,0.06), rgba(180,130,20,0.06));
      ...
      .resolution-card:hover {
        ...
        background: linear-gradient(135deg, rgba(218,165,32,0.1), rgba(180,130,20,0.1));
      }
