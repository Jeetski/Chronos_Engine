...
export function mount(el, context){
  ...
  function positionFloating(el, left, top){
    ...
    const maxTop = Math.max(48, window.innerHeight - height - pad);
    const safeLeft = clamp(left, pad, maxLeft);
    const safeTop = clamp(top, 48, maxTop);
  ...
  function syncFloatingNote(note){
    ...
    if (!entry) return;
    const el = entry.el;
    el.dataset.color = note.color || DEFAULT_COLOR;
    const title = el.querySelector('.sticky-float-title');
    if (title) title.textContent = note.name;
    const footer = el.querySelector('.sticky-float-footer span');
    if (footer) footer.textContent = formatUpdated(note.updated);
  ...
  function ensureFloatingNote(note){
    ...
    el.className = 'sticky-float-note';
    el.dataset.color = note.color || DEFAULT_COLOR;
    ...
    el.innerHTML = `
      ...
      <div class="sticky-float-footer">
        <span>${formatUpdated(note.updated)}</span>
      </div>
    `;
    const textarea = el.querySelector('textarea');
    if (textarea) {
    ...
    ordered.forEach(note => {
      ...
      card.innerHTML = `
        <div class="sticky-note-header">
          <span class="sticky-note-title" title="Drag to float">${note.name}</span>
      ...
      `;
      const dragHandle = card.querySelector('.sticky-note-header');
      if (dragHandle) {
        dragHandle.setAttribute('data-no-drag', 'true');
        dragHandle.title = 'Drag to float';
        wireFloatDrag(dragHandle, note);
      }
