...
def list_matrix_presets() -> List[Dict[str, Any]]:
    ...
    presets: Dict[str, Dict[str, Any]] = {
        slug: deepcopy(payload) for slug, payload in BUILTIN_PRESET_INDEX.items()
    }
    ...
    for item_type in DEFAULT_ITEM_TYPES:
        ...
        for entry in entries:
            if not isinstance(entry, dict):
                continue
            record_type = _normalize_key(entry.get("type")) or _normalize_key(item_type)
            dataset.append(
                {
                    "name": _normalize_value(entry.get("name") or ""),
                    "type": record_type,
