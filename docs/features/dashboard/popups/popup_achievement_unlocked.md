...
## Overview
Celebratory unlock popup that surfaces recent achievement awards and progression context.
Popups should be concise, actionable, and dismissible without blocking core workflow.

## CLI
- Related command patterns: `achievements sync; achievements event; points balance; view achievement <name>`.
- CLI is the fallback path for actions suggested by popup prompts.
- Command reference: `docs/reference/cli_commands.md`.

## Dashboard
- Runtime source: `utilities/dashboard/popups/achievement_unlocked/`
...
## Validation
1. Trigger `AchievementUnlocked` popup intentionally in test flow.
