...
## Overview
Today agenda control widget for schedule view and command-driven edits.
Widgets should keep UI actions aligned with canonical CLI/API behavior and avoid hidden side effects.

## CLI
- Related command patterns: `today; today reschedule; trim; change; split; merge; anchor; mark; did; quickwins`.
...
## Dashboard
- Runtime source: `utilities/dashboard/widgets/today/`
- Primary role: focused operational UI for this feature domain.
- API endpoints used by this widget:
  - `/api/cli`
  - `/api/settings?file=Scheduling_Priorities.yml`
  - `/api/today`
