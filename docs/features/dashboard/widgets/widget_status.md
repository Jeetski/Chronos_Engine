...
## Overview
Current status display/update widget tied to status settings taxonomy.
Widgets should keep UI actions aligned with canonical CLI/API behavior and avoid hidden side effects.

## CLI
- Related command patterns: `status; status key:value; settings status files`.
...
## Dashboard
- Runtime source: `utilities/dashboard/widgets/status/`
