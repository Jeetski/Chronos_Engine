...
def run(args, properties):
    """
    Create an alarm from an item's deadline or due date.
    ...
    if not date:
        ...
        return
    if not time:
        defaults = _load_defaults("alarm")
        time = defaults.get("time") or "09:00"

    base_suffix = "deadline alarm" if date_kind == "deadline" else "due alarm"
    alarm_name = _unique_item_name("alarm", f"{item_name} {base_suffix}")
    ...
    write_item_data("alarm", alarm_name, data)
    print(f"? Created alarm: {alarm_name}.yml")


def get_help_message():
    return """
Usage: alarm from <type> <name> [use:deadline|due_date] [date:YYYY-MM-DD] [time:HH:MM] [message:"..."] [recurrence:daily]
Description:
  Creates an alarm from an item's deadline or due_date.
Examples:
