...
def stretch_item_in_file(file_path, item_name, amount_minutes):
    """
    Records a stretch modification for the specified item.
    """
    manual_path = manual_modifications_path_for_schedule(file_path)
    modifications = load_manual_modifications(manual_path)
    modifications.append({"action": "stretch", "item_name": item_name, "amount": amount_minutes})
    save_manual_modifications(modifications, manual_path)
    print(f"✅ Recorded stretch for '{item_name}' by {amount_minutes} minutes. Run 'today reschedule' to apply changes.")
    return True

def anchor_item_in_file(file_path, item_name, scope="today"):
    """
    Records an anchor modification for the specified item (today-only).
    """
    manual_path = manual_modifications_path_for_schedule(file_path)
    modifications = load_manual_modifications(manual_path)
    modifications.append({"action": "anchor", "item_name": item_name, "scope": scope})
    save_manual_modifications(modifications, manual_path)
    print(f"✅ Recorded anchor for '{item_name}' (scope: {scope}). Run 'today reschedule' to apply changes.")
    return True

def split_item_in_file(file_path, item_name, count=2):
    """
    Records a split modification for the specified item.
    """
    manual_path = manual_modifications_path_for_schedule(file_path)
    modifications = load_manual_modifications(manual_path)
    modifications.append({"action": "split", "item_name": item_name, "count": count})
    save_manual_modifications(modifications, manual_path)
    print(f"✅ Recorded split for '{item_name}' into {count} parts. Run 'today reschedule' to apply changes.")
    return True

def merge_item_in_file(file_path, item_name, other_name):
    """
    Records a merge modification for two specified items.
    """
    manual_path = manual_modifications_path_for_schedule(file_path)
    modifications = load_manual_modifications(manual_path)
    modifications.append({"action": "merge", "item_name": item_name, "other_name": other_name})
    save_manual_modifications(modifications, manual_path)
    print(f"✅ Recorded merge for '{item_name}' with '{other_name}'. Run 'today reschedule' to apply changes.")
    return True
...
def apply_manual_modifications(schedule, manual_modifications):
    ...
    for mod in manual_modifications:
        ...
        if action == "trim":
            ...
            start_time_str = mod.get("start_time")
            item_type = mod.get("item_type", "task")
            if not start_time_str:
                continue
                
            try:
                h, m = map(int, start_time_str.split(':'))
                new_start_dt = datetime.now().replace(hour=h, minute=m, second=0, microsecond=0)
            except:
                continue

            # Check if already exists to avoid dupes?
            # existing, _ = find_item_in_schedule(schedule, item_name)
            # if existing: continue 

            # Create item
            # Try to read real data first
            real_data = read_item_data(item_type, item_name) or {}
            duration_str = real_data.get("duration", "15m")
            duration_mins = parse_duration_string(duration_str)
            ...
            new_item = {
                ...
                "end_time": new_start_dt + timedelta(minutes=duration_mins),
                "original_item_data": real_data,
                "children": [],
                "injected": True
            }
            schedule.append(new_item)
            print(f"✅ Applied manual injection for '{item_name}' at {start_time_str}.")
            ...
            item["end_time"] = item["start_time"] + timedelta(minutes=item["duration"])
            update_parent_times(item)
            print(f"✅ Applied manual stretch for '{item_name}' by {amount} minutes.")

        elif action == "anchor":
            scope = str(mod.get("scope") or "today").lower()
            item["reschedule"] = "never"
            item["anchored"] = True
            update_parent_times(item)
            print(f"✅ Applied manual anchor for '{item_name}' (scope: {scope}).")
            ...
            try:
                ...
                count = 2
            if count < 2:
                print(f"⚠️ Warning: Split count must be >= 2 for '{item_name}'. Skipping.")
                continue
            if item.get("duration", 0) < count * 5:
                print(f"⚠️ Warning: Cannot split '{item_name}' into {count} parts (min 5m each). Skipping.")
                continue
            ...
            if parent_item and "children" in parent_item:
                ...
                parent_list = schedule

            try:
                idx = parent_list.index(item)
            except ValueError:
                idx = None

            part_minutes = int(item["duration"] // count)
            remainder = int(item["duration"] % count)
            parts = []
            cursor = item["start_time"]

            for i in range(count):
                extra = 1 if i < remainder else 0
                minutes = part_minutes + extra
                part_name = f"{item_name} ({i+1}/{count})"
                new_item = dict(item)
                new_item["name"] = part_name
                new_item["duration"] = minutes
                new_item["start_time"] = cursor
                new_item["end_time"] = cursor + timedelta(minutes=minutes)
                new_item["children"] = []
                new_item["parent"] = parent_item
                parts.append(new_item)
                cursor = new_item["end_time"]

            if idx is not None:
                parent_list.pop(idx)
                for offset, part in enumerate(parts):
                    parent_list.insert(idx + offset, part)
            else:
                parent_list.extend(parts)

            update_parent_times(parent_item or parts[0])
            print(f"✅ Applied manual split for '{item_name}' into {count} parts.")

        elif action == "merge":
            other_name = mod.get("other_name")
            if not other_name:
                continue
            other_item, other_parent = find_item_in_schedule(schedule, other_name)
            if not other_item:
                print(f"⚠️ Warning: Merge target '{other_name}' not found. Skipping.")
                continue
            if other_parent != parent_item:
                print(f"⚠️ Warning: Merge requires items with the same parent. Skipping '{item_name}' + '{other_name}'.")
                continue

            start_time = min(item.get("start_time"), other_item.get("start_time"))
            end_time = max(item.get("end_time"), other_item.get("end_time"))
            item["start_time"] = start_time
            item["end_time"] = end_time
            try:
                item["duration"] = int((end_time - start_time).total_seconds() / 60)
            except Exception:
                pass

            parent_list = parent_item.get("children") if parent_item else schedule
            try:
                parent_list.remove(other_item)
            except Exception:
                pass
            update_parent_times(parent_item or item)
            print(f"✅ Applied manual merge for '{item_name}' with '{other_name}'.")

    return schedule


from modules.item_manager import read_item_data, list_all_items_any
...
def display_schedule(schedule, conflicts, indent=0, display_level=float('inf'), current_level=0, today_completion_data=None, color_override=None, title=None):
    ...
    with color coding based on completion status using console styling.
    """
    indent_str = "  " * indent
    if indent == 0:
        # Use console styling for the header as well
    ...
    for item in schedule:
        if item.get("is_buffer"):
            console_style.print_role(f"{indent_str}--- Buffer ({item['duration']} minutes) ---", "status.buffer")
        else:
            ...
            text_role = STATUS_ROLE_MAP.get(status_key, "error")
    ...
    if indent == 0 and conflicts:
        console_style.print_role("\n--- Conflicts ---", "header")
        for conflict in conflicts:
            console_style.print_role(conflict, "error")
