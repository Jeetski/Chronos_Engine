...
function filterDueSoon(items) {
  ...
  return items
    .map(it => {
      ...
      const dueRaw = deadline || it.due_date || it.due || it.date;
      const date = parseDate(dueRaw);
      return {
        ...it,
        due: dueRaw,
        due_kind: deadline ? 'deadline' : 'due_date',
        __date: date,
      };
    })
    .filter(it => {
    ...
    .sort((a, b) => {
      ...
      if (!aOverdue && bOverdue) return 1;
      if (a.__date - b.__date !== 0) return a.__date - b.__date;
      if (a.due_kind === 'deadline' && b.due_kind !== 'deadline') return -1;
      if (a.due_kind !== 'deadline' && b.due_kind === 'deadline') return 1;
      return 0;
    });
}
    ...
    return `
      <li class="due-soon-item">
        <span>
          ...
          <span class="due-soon-meta">${(it.type || '').toUpperCase()} ${it.due_kind === 'deadline' ? 'DEADLINE' : 'DUE'}</span>
        </span>
        <span class="due-soon-due">${dateStr} (${dueLabel})</span>
      </li>
    `;
  }).join('') || `<li class="due-soon-item"><span class="due-soon-meta">No items due soon.</span></li>`;
  wrap.innerHTML = `
...
async function maybeShowDueSoon(done) {
  ...
  if (!MANUAL_LAUNCH && !dueSoon.length) { done?.(); return; }
  injectStyles();
