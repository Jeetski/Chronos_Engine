export async function mount(el, context) {
  const TEMPLATE_TYPES = ['routine', 'subroutine', 'microroutine'];
  const TYPES = ['routine','subroutine','microroutine','habit','task','window','buffer','break'];
  const ALIAS = { chore: 'habit', habit_stack: 'microroutine', 'habit stack': 'microroutine' };
  const COLORS = { task:'#7aa2f7', habit:'#86efac', routine:'#f59e0b', subroutine:'#f97316', microroutine:'#fb7185', window:'#38bdf8', buffer:'#94a3b8', break:'#22d3ee' };
  ...
  el.innerHTML = `
    <style>
      ...
      @media (max-width:1040px){.db{grid-template-columns:minmax(240px,300px) minmax(0,1fr)} #right{grid-column:1 / -1}}
      @media (max-width:820px){.db{grid-template-columns:1fr} #right{grid-column:auto}}
    </style>
    <div class="db">
  ...
  `;
  mid.innerHTML = `
    <div class="r" style="justify-content:space-between"><div class="title">Routine Builder</div><div class="sub">Template composition view</div></div>
    <details class="mid-controls" id="midControls" open>
      ...
      <div class="toolbar">
        <div class="group grow">
          ...
          <select id="rtType" class="sel" style="width:140px"></select>
  ...
  right.innerHTML = `
    ...
    <div class="sub">Template properties</div>
  ...
  const rtType = el.querySelector('#rtType');
  ...
  TEMPLATE_TYPES.forEach((t) => {
    ...
    o.value = t;
    o.textContent = t;
    if (t === state.templateType) o.selected = true;
    rtType.appendChild(o);
  });
  ...
  const loadPalette = async () => {
    ...
    const templateTypes = ['routine', 'subroutine', 'microroutine', 'window'];
  ...
  const loadTemplateList = async () => {
    try {
      const p = await jget(`/api/template/list?type=${encodeURIComponent(state.templateType)}`);
  ...
  const loadDayTemplate = async (name) => {
    const n = String(name || '').trim(); if (!n) { emit('error', `Choose a ${state.templateType} template first.`); return; }
    try {
      const p = await jget(`/api/template?type=${encodeURIComponent(state.templateType)}&name=${encodeURIComponent(n)}`);
      ...
      drawTimeline(); drawInspector(); emit('ok', `Loaded ${state.templateType} template: ${n}`);
  ...
  const saveDayTemplate = async (saveAs) => {
    ...
    if (saveAs || !n) { n = String(prompt('Template name:', n || '') || '').trim(); if (!n) return false; }

    const children = buildHierarchyChildren();

    const r1 = await jpost('/api/template', { type: state.templateType, name: n, children });
    if (!r1.ok) { emit('error', r1?.j?.error || 'Could not save template children.'); return false; }
    const r2 = await jpost('/api/item', { type: state.templateType, name: n, properties: getTemplateProps() });
    if (!r2.ok) { emit('error', r2?.j?.error || 'Children saved, property update failed.'); return false; }

    state.templateName = n; await loadTemplateList(); tName.value = n; emit('ok', `Saved ${state.templateType} template: ${n}`);
    return true;
  };

  const newDayTemplate = async () => {
    const suggested = String(state.templateName || tName.value || '').trim();
    const n = String(prompt(`New ${state.templateType} template name:`, suggested ? `${suggested} copy` : '') || '').trim();
    if (!n) return;

    const r = await jpost('/api/template', { type: state.templateType, name: n, children: [] });
    if (!r.ok) {
  ...
  const renameTemplate = async () => {
    ...
    const r = await jpost('/api/item/rename', { type: state.templateType, old_name: oldName, new_name: next });
  ...
  const deleteTemplate = async () => {
    ...
    if (!confirm(`Delete ${state.templateType} template "${name}"?`)) return;
    const r = await jpost('/api/item/delete', { type: state.templateType, name });
  ...
  const qaCheck = () => {
    ...
    emit('ok', 'Routine Builder QA check complete.');
  };

  pSearch.oninput = () => drawPalette();
  rtType.onchange = async () => {
    state.templateType = String(rtType.value || 'routine');
    state.templateName = '';
    await loadTemplateList();
    state.schedule = [];
    state.selected = -1;
  ...
  el.querySelector('#copy').onclick = async () => {
    try {
      const payload = { template: { type: state.templateType, name: state.templateName || String(tName.value || ''), properties: getTemplateProps() }, items: state.schedule };
      await navigator.clipboard.writeText(JSON.stringify(payload, null, 2));
      emit('ok', 'Routine Builder draft copied to clipboard.');
    } catch { emit('error', 'Could not copy draft JSON.'); }
  };

  rtType.value = state.templateType;
