...
# --- Handler Functions for 'day' Item Type ---

def handle_new(name, properties):
    """
    Handles the creation of a new day template item.
    Initializes with default properties and an empty sequence.
    """
    # Default properties for a new day
    content = {
        "Name": name,
        "Description": f"A new {ITEM_TYPE} template.",
        "Category": "General",
        "Duration": "24h",
        "Sequence": [] # Initialize with an empty sequence of routines
    }
...
def handle_view(name, properties):
    """
    Handles viewing the content and properties of a day template item.
    """
    ...
    for key, value in data.items():
        if key == "Sequence":
            ...
            if value:
                for i, routine_entry in enumerate(value):
                    print(f"  {i+1}. Routine:")
                    for routine_key, routine_value in routine_entry.get("Routine", {}).items():
                        print(f"     {routine_key}: {routine_value}")
            else:
...
def handle_edit(name, properties):
    """
    Handles opening a day template item in a text editor.
    """
...
def handle_list(sort_by, properties):
    """
    Handles listing day template items.
    """
...
def handle_delete(name, properties):
    """
    Handles deleting a day template item, with optional force confirmation.
    """
