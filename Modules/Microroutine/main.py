...
ITEM_TYPE = "microroutine"

# --- Handler Functions for 'microroutine' Item Type ---

def handle_new(name, properties):
    """
    Handles the creation of a new microroutine template item.
    Initializes with default properties and an empty sequence.
    """
    # Default properties for a new microroutine
    content = {
        "Name": name,
        "Description": f"A new {ITEM_TYPE} template.",
        "Category": "General",
        "Duration": 0,
        "Sequence": [] # Initialize with an empty sequence
    }
...
def handle_view(name, properties):
    """
    Handles viewing the content and properties of a microroutine template item.
    """
    ...
    for key, value in data.items():
        if key == "Sequence":
            print(f"\n--- Sequence ---")
            if value:
                for i, item_entry in enumerate(value):
                    print(f"  {i+1}. Item:")
                    for item_key, item_value in item_entry.get("Item", {}).items():
...
def handle_edit(name, properties):
    """
    Handles opening a microroutine template item in a text editor.
    """
...
def handle_list(sort_by, properties):
    """
    Handles listing microroutine template items.
    """
...
def handle_delete(name, properties):
    """
    Handles deleting a microroutine template item, with optional force confirmation.
    """
