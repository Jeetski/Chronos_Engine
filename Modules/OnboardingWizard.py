    ...
    def step_current_status(self):
        ...
        for entry in statuses:
            ...
            if options:
                ...
                if choice:
                    normalized, err = status_utils.canonicalize_status_value(key, choice)
                    if err:
                        print(f"Warning: {err}")
                    else:
                        data[key] = normalized
                ...
                if value:
                    normalized, err = status_utils.canonicalize_status_value(key, value)
                    if err:
                        print(f"Warning: {err}")
                    else:
                        data[key] = normalized
        self.save_yaml(path, data)
        history_path = Path(status_history_path_for_date(datetime.now()))
        history = self.load_yaml(history_path, {}) if history_path.exists() else {}
        ...
        self.save_yaml(history_path, history)
