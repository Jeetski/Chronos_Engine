...
ITEM_TYPE = "subroutine"

# --- Handler Functions for 'subroutine' Item Type ---

def handle_new(name, properties):
    """
    Handles the creation of a new subroutine template item.
    Initializes with default properties and an empty sequence.
    """
    # Default properties for a new subroutine
    content = {
        "Name": name,
        "Description": f"A new {ITEM_TYPE} template.",
        "Category": "General",
        "Duration": "0m",
        "Sequence": [] # Initialize with an empty sequence of microroutines
    }
...
def handle_view(name, properties):
    """
    Handles viewing the content and properties of a subroutine template item.
    """
    ...
    for key, value in data.items():
        if key == "Sequence":
            print(f"\n--- Sequence of Microroutines ---")
            if value:
                for i, microroutine_entry in enumerate(value):
                    print(f"  {i+1}. Microroutine:")
                    for micro_key, micro_value in microroutine_entry.get("Microroutine", {}).items():
                        print(f"     {micro_key}: {micro_value}")
            else:
...
def handle_edit(name, properties):
    """
    Handles opening a subroutine template item in a text editor.
    """
...
def handle_list(sort_by, properties):
    """
    Handles listing subroutine template items.
    """
