...
# --- Handler Functions for 'routine' Item Type ---

def handle_new(name, properties):
    """
    Handles the creation of a new routine template item.
    Initializes with default properties and an empty sequence.
    """
    # Default properties for a new routine
    content = {
        "Name": name,
        "Description": f"A new {ITEM_TYPE} template.",
        "Category": "General",
        "Duration": 0,
        "Sequence": [] # Initialize with an empty sequence of subroutines
    }
...
def handle_view(name, properties):
    """
    Handles viewing the content and properties of a routine template item.
    """
    ...
    for key, value in data.items():
        if key == "Sequence":
            ...
            if value:
                for i, subroutine_entry in enumerate(value):
                    print(f"  {i+1}. Subroutine:")
                    for sub_key, sub_value in subroutine_entry.get("Subroutine", {}).items():
...
def handle_edit(name, properties):
    """
    Handles opening a routine template item in a text editor.
    """
...
def handle_list(sort_by, properties):
    """
    Handles listing routine template items.
    """
...
def handle_delete(name, properties):
    """
    Handles deleting a routine template item, with optional force confirmation.
    """
